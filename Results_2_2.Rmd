...
```{r message=FALSE, warning=FALSE, include=FALSE}
results_dir <- "results/section2.2/"
...
```{r}
sum(table(orftable_annotated$`ORF type`))
...
orftype_codon_data <- data.frame(table(orftable_annotated$`ORF type`, orftable_annotated$`Start codon`)) %>%
...
summary_start <- table(orftable_annotated$`ORF type`, orftable_annotated$`Start codon`)
