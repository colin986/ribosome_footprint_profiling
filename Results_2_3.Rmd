...
```{r}
results_dir <- "results/section2.3/"
...
```{r}
load("results/orftable/ortable.rData")
```

###  ORF length plot
```{r}
source(file = "results/r_scripts/facetzoom2.R")
safe_colorblind_palette <- c(
  "#88CCEE", "#CC6677", "#888888", "#117733", "#332288", "#035968",
  "#AA4499", "#999933", "#882255", "#661100", "#6699CC", "#888888"
)

orf_length_plot <- orftable_annotated %>%
...
```{r}
uorf_length_plot <- orftable_annotated %>%
...
ouorf_length_plot <- orftable_annotated %>%
...
new_orf_length_plot <- orftable_annotated %>%
  filter(`ORF type` == "New") %>%
  ggplot(aes(x = sort(`Length (AAs)`))) +
...
### sORF proportion
```{r}
all_short_orfs <- orftable_annotated %>%
  filter(`Length (AAs)` < 100)

table(all_short_orfs$`ORF type`) / sum(table(all_short_orfs$`ORF type`))


table(all_short_orfs$`ORF type`) / table(orftable_annotated$`ORF type`)
...
table(sORF_codon$length_classification, sORF_codon$`ORF type`)


proportion_sorf <- orftable_annotated %>%
  mutate(length_classification = case_when(
    `Length (AAs)` >= 100 ~ ">= 100aa",
    TRUE ~ "< 100aa"
  )) %>%
