...
source("orf_filtering_functions.R")
```
...
paste0(dim(orftable)[1], " remain following removal of truncated, internal, LOOF and low confidence isoforms")
```
...
orftable_annotated <- bind_cols(orftable_annotated,
  ...
  `ORF type` != "Annotated" & Rharr_min_Rnd >= 0.03 ~ "TRUE",
...
table(orftable_annotated$`ORF type`, orftable_annotated$`Start codon`)
```
...
orftable_annotated <- orftable_annotated %>% 
  ...
  mutate(orfik_id = ave(`Transcript ID`, `Transcript ID`, FUN = function(x) ifelse(duplicated(x), 
...
### Classify novel ORFs
```{r}
orftable_annotated$floss_classification <- as.character(classifier_ref$classify(orftable_annotated$countRFP, orftable_annotated$floss))
...
```{r}
system("mkdir ../../results/orftable")
save(orftable_annotated, file="../../results/orftable/ortable.rData")
```
...
proteoform_annotations <- orftable_annotated %>%
  filter(`ORF type` != "Annotated" ) %>%
  filter(`Length (AAs)` >= 100)

table(proteoform_annotations$`ORF type`, proteoform_annotations$`Start codon`)

cds_novel_orfs <- loadRegion(txdb_orf, part = "cds")
orf_to_fasta(microprotein_annotations, cds_novel_orfs, "microprotein_aa", cgr_fasta)
orf_to_fasta(proteoform_annotations, cds_novel_orfs, "proteoform_aa", cgr_fasta)

system("cat ../../results/FASTA/UP000001075_10029.fasta ../../results/FASTA/microprotein_aa.fasta ../../results/FASTA/proteoform_aa.fasta  > ../../results/FASTA/270723_uniprot_and_novel.fasta", intern = T)
```
...
system("mkdir ../../results/DE_analysis", intern = T,ignore.stderr = T, ignore.stdout = T)
write(selected_new_for_ts_de$`ORF-RATER name`, file="../../results/DE_analysis/lncRNA_novel_orfs.txt")
```
