...
hcp_bar_plot_data <- bind_rows(
  ...
  data.frame(sample = "adalimumab", table(ada_dp_nibrt_class$prot_class))
)

hcp_bar_plot <- hcp_bar_plot_data %>%
  filter(sample == "adalimumab") %>%
...
hcp_bar_plot
ggsave(hcp_bar_plot, filename = paste0(results_dir,"Figure 4C.png"), device = "png", dpi = 700, width = 8, height = 2)
```
...
```{r}
dp_sorf_table <- bind_rows(ada_dp_nibrt_sorf)
...
microprotein_dp_table <- tableGrob(dp_sorf_table, theme = ttheme_default(
  ...
  core = list(
    bg_params = list(fill = c("grey95", "grey90", "grey95", "grey90","#35968CFF", "grey95",  "grey95", "grey90", "grey95", "grey90", "grey95")),
    fg_params = list(col = c("black", "black", "black", "black", "white","black",  "black", "black", "black", "black", "black"))
  )
))
