...
transcript_counts %>%
  filter(`Transcript ID` == "XM_027423276.2")
```
...
```{r}
orftable_annotated_l1_stringent <- bind_cols(orftable_annotated,
...
microproteins_l1_stringent <- orftable_annotated_l1_stringent %>%
  filter(`ORF type` == "New" | `ORF type` == "Upstream" | `ORF type` == "Start overlap") %>%
  filter(`Length (AAs)` < 100) 


orf_to_fasta(microproteins_l1_stringent, cds_novel_orfs, "microproteins_l1_stringent", cgr_fasta)

microproteins_l1_stringent_gr <- cds_novel_orfs[names(cds_novel_orfs) %in% microproteins_l1_stringent$`ORF-RATER name`]
```

```{r}
orftable_annotated_l1_relaxed <- bind_cols(orftable_annotated,
                            data.frame(xharr = orf_tis_counts_harr,
                                       xnd = orf_tis_counts_nd)) %>%
  left_join(transcript_counts, by = "Transcript ID") %>%
  mutate(Rharr = xharr/nharr,
...
microproteins_l1_relaxed <- orftable_annotated_l1_relaxed %>%
  filter(`ORF type` == "New" | `ORF type` == "Upstream" | `ORF type` == "Start overlap") %>%
  filter(`Length (AAs)` < 100) 

orftable_annotated_l1_relaxed %>%
  filter(`ORF-RATER name` %in% unique_hcps_relaxed)


orf_to_fasta(microproteins_l1_relaxed, cds_novel_orfs, "microproteins_l1_relaxed", cgr_fasta)
```
...
orftable_annotated_l1_stringent %>%
  filter(`ORF-RATER name` %in% unique_hcps_stringent)
```
...
```{r}
orftable_annotated <- orftable_annotated_l1_stringent
...
```{r}
non_truncates <-orftable_annotated %>% 
  filter(`ORF type` != "Upstream") %>% 
  filter(`ORF type` != "Start overlap") %>%
  filter(`ORF type` != "New") 

# filter uORFs
single_proteoform_transcripts <- orftable_annotated %>% 
...
```{r}
uORF_filtered <- orftable_annotated %>%
  filter(`ORF type` == "Upstream") %>%
  group_by(`Transcript ID`) %>%
  mutate(count=n()) %>%
  filter(count > 1) %>%
...
ouORF_filtered <- orftable_annotated %>%
  filter(`ORF type` == "Start overlap") %>%
  group_by(`Transcript ID`) %>%
  mutate(count=n()) %>%
  filter(count > 1) %>%
...
new_filtered <- orftable_annotated %>%
  filter(`ORF type` == "New") %>%
  group_by(`Transcript ID`) %>%
  mutate(count=n()) %>%
  filter(count > 1) %>%
